...
from lib.database import DBHandler, PendingTransaction
            ...
            if field is None:
                ...
                try:
                    worksheet.cell(row=r_index, column=f_index).value = field
                            ...
                            worksheet.cell(row=r_index, column=f_index).value = unidecode(unidecode(field).decode('utf-8'))
...
def main():
    ...
    for organization in get_items(config, "organizations", params, session):
        memberships = list(get_memberships(organization=organization, config=config, session=session))
    ...
    for department in departments:
        queue.put(process_departments(departments, config))
